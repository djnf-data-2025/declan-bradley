...
bp_fy2020 <- read_csv("r_intro/load_and_analyze/bp_fy_2020.csv")
...
In what season of the year are apprehensions most common?
```{r}
bp_fy2020 |>
  mutate(
  ...
  select(sector, fall, winter, spring, summer) |>
  pivot_longer(cols = c(winter, spring, summer, fall), names_to = "season", values_to = "apprehensions") |>
  group_by(season) |>
  summarize(total_apprehensions = sum(apprehensions)) |>
  arrange(desc(total_apprehensions))
```
They are most common in fall

What percentage of apprehensions occurred in fall?
```{r}
bp_fy2020 |>
  mutate(
    fall = september + october + november,
  ...
  select(sector, fall, winter, spring, summer) |>
  pivot_longer(cols = fall:summer, names_to = "season", values_to = "apprehensions") |>
  group_by(season) |>
  summarize(total_apprehensions = sum(apprehensions)) |>
  arrange(desc(total_apprehensions)) |>
  mutate(perc_of_year = total_apprehensions / sum(total_apprehensions))
```
about 31 percent